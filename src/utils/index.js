...
export const Error = (msg) => {
    Message.create({ content: msg, color : 'danger' ,duration:2  });
}

export const Warning = (msg) => {
    Message.create({ content: msg, color : 'warning',duration:2  });
}
/**
 * 数据返回统一处理函数
 * @param {*} response 
...
export const processData = (response,successMsg) => {
    ...
    if(response.status=='200'){
        ...
        let repMsg = data.msg;//根据返回的数据格式赋值
        if(repMsg=='success'){   
            if(successMsg){
                success(successMsg);
            }
            return data.data;//根据返回的数据格式赋值
